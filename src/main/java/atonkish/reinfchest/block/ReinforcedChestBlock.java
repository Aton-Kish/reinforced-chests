...
public class ReinforcedChestBlock extends ChestBlock {
  private static final Map<
          ReinforcingMaterial,
          DoubleBlockProperties.PropertyRetriever<
              ChestBlockEntity, Optional<NamedScreenHandlerFactory>>>
      NAME_RETRIEVER_MAP = new LinkedHashMap<>();
  private final ReinforcingMaterial material;

  protected ReinforcedChestBlock(
  ...
  protected static DoubleBlockProperties.PropertyRetriever<
          ChestBlockEntity, Optional<NamedScreenHandlerFactory>>
      registerMaterialNameRetriever(ReinforcingMaterial material) {
    if (!NAME_RETRIEVER_MAP.containsKey(material)) {
      DoubleBlockProperties.PropertyRetriever<ChestBlockEntity, Optional<NamedScreenHandlerFactory>>
          nameRetriever =
              new DoubleBlockProperties.PropertyRetriever<
                ...
                public Optional<NamedScreenHandlerFactory> getFromBoth(
                  ...
                  final Inventory inventory =
                      new DoubleInventory(chestBlockEntity, chestBlockEntity2);
                  return Optional.of(
                      new NamedScreenHandlerFactory() {
                        @Nullable public ScreenHandler createMenu(
                          ...
                          if (chestBlockEntity.checkUnlocked(playerEntity)
                            ...
                            return ReinforcedStorageScreenHandler.createDoubleBlockScreen(
                                material, i, playerInventory, inventory);
                          } else {
                            return null;
                          }
                        }
                        ...
                        public Text getDisplayName() {
                          if (chestBlockEntity.hasCustomName()) {
                            ...
                            String namespace =
                                BlockEntityType.getId(chestBlockEntity.getType()).getNamespace();
                            return (Text)
                                (chestBlockEntity2.hasCustomName()
                                    ? chestBlockEntity2.getDisplayName()
                                    : Text.translatable(
                                        "container."
                                            + namespace
                                            + "."
                ...
                public Optional<NamedScreenHandlerFactory> getFrom(
  ...
  @Override
  @Nullable public NamedScreenHandlerFactory createScreenHandlerFactory(
      BlockState state, World world, BlockPos pos) {
    return ((Optional<NamedScreenHandlerFactory>)
            this.getBlockEntitySource(state, world, pos, false)
                .apply(NAME_RETRIEVER_MAP.get(this.material)))
        .orElse((NamedScreenHandlerFactory) null);
  }
