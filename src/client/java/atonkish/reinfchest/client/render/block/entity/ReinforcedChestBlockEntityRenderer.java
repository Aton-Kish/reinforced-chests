...
public class ReinforcedChestBlockEntityRenderer<T extends BlockEntity & LidOpenable>
  ...
  public ReinforcedChestBlockEntityRenderer(BlockEntityRendererFactory.Context context) {
    ...
    this.doubleChestLeft =
        new ChestBlockModel(context.getLayerModelPart(EntityModelLayers.DOUBLE_CHEST_LEFT));
    this.doubleChestRight =
        new ChestBlockModel(context.getLayerModelPart(EntityModelLayers.DOUBLE_CHEST_RIGHT));
  }
  ...
  public void updateRenderState(
    ...
    BlockEntityRenderState.updateBlockEntityRenderState(
        blockEntity, chestBlockEntityRenderState, crumblingOverlayCommand);
    ...
    chestBlockEntityRenderState.chestType =
        blockState.contains(ChestBlock.CHEST_TYPE)
            ? (ChestType) blockState.get(ChestBlock.CHEST_TYPE)
            : ChestType.SINGLE;
    chestBlockEntityRenderState.yaw =
        ((Direction) blockState.get(ChestBlock.FACING)).getPositiveHorizontalDegrees();
    ...
    if (bl && blockState.getBlock() instanceof ReinforcedChestBlock chestBlock) {
      propertySource =
          chestBlock.getBlockEntitySource(
              blockState, blockEntity.getWorld(), blockEntity.getPos(), true);
    } else {
    ...
    chestBlockEntityRenderState.lidAnimationProgress =
        ((Float2FloatFunction)
                propertySource.apply(
                    ChestBlock.getAnimationProgressRetriever((LidOpenable) blockEntity)))
            .get(f);
    if (chestBlockEntityRenderState.chestType != ChestType.SINGLE) {
      chestBlockEntityRenderState.lightmapCoordinates =
          ((Int2IntFunction) propertySource.apply(new LightmapCoordinatesRetriever<>()))
  ...
  public void render(
    ...
    ReinforcingMaterial material =
        ((ReinforcedChestBlock) chestBlockEntityRenderState.blockState.getBlock()).getMaterial();
    SpriteIdentifier spriteIdentifier =
        ModTexturedRenderLayers.getReinforcedChestTextureId(
            material, chestBlockEntityRenderState.variant, chestBlockEntityRenderState.chestType);
  ...
  private ChestBlockEntityRenderState.Variant getVariant(
      BlockEntity blockEntity, boolean christmas) {
    if (christmas) {
