...
public class ModTexturedRenderLayers {
  public static final Map<ReinforcingMaterial, SpriteIdentifier> REINFORCED_CHEST_SINGLE_MAP =
      new LinkedHashMap<>();
  public static final Map<ReinforcingMaterial, SpriteIdentifier> REINFORCED_CHEST_LEFT_MAP =
      new LinkedHashMap<>();
  public static final Map<ReinforcingMaterial, SpriteIdentifier> REINFORCED_CHEST_RIGHT_MAP =
      new LinkedHashMap<>();
  ...
  private static SpriteIdentifier getReinforcedChestTextureSpriteIdentifier(
      String namespace, ReinforcingMaterial material, String variant) {
    Identifier textureId =
        Identifier.of(namespace, String.format("entity/chest/%s/%s", material.getName(), variant));
    return new SpriteIdentifier(TexturedRenderLayers.CHEST_ATLAS_TEXTURE, textureId);
  }

  public static SpriteIdentifier getReinforcedChestTextureId(
      ReinforcingMaterial material, ChestBlockEntityRenderState.Variant variant, ChestType type) {
    switch (variant) {
  ...
  private static SpriteIdentifier getReinforcedChestTextureId(
      ChestType type, SpriteIdentifier single, SpriteIdentifier left, SpriteIdentifier right) {
    switch (type) {
      case LEFT:
        return left;
      case RIGHT:
